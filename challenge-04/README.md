...
carro.adicionarPessoas = function(nPessoas) {
    ...
    var a = vagas > 1 ? 'pessoas' : 'pessoa';
    if(this.quantiadePessoas === this.assentos) return 'O carro já está lotado!';
    if(vagas < nPessoas) {
        return `Só cabem mais ${vagas} ${a}!`;
    } else {
        this.quantidadePessoas += nPessoas;
    }
...
carro.obterMarcaModelo() //'Esse carro é um Volkswagen Jetta'
